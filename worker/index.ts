...
async function debugYouTube(request: Request, env: Env): Promise<Response> {
  ...
  // 端点的实际协议（HTTP CONNECT vs SOCKS5）未知，所以两种握手都试一次。
  const proxy = loadProxyConfig(env as unknown as Record<string, string | undefined>);
  if (proxy) {
    // 3a: HTTP CONNECT 协议
    try {
      const r = await fetchYouTubeSubtitles(videoId, { proxy, proxyProtocol: 'http-connect' });
      tests.proxyHttpConnect = {
        success: true,
    ...
    // 3b: SOCKS5 协议
    try {
      const r = await fetchYouTubeSubtitles(videoId, { proxy, proxyProtocol: 'socks5' });
      tests.proxySocks5 = {
        success: true,
        proxyHost: `${proxy.host}:${proxy.port}`,
        languageCode: r.languageCode,
